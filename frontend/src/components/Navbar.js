    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-12 text-sm font-medium text-gray-700">
          ...
          {token && <Link to="/wishlist" className="hover:text-black">Wishlist</Link>}
        ...
        <div className="flex items-center gap-5">
          ...
          {!token ? (
            <Link to="/login" className="bg-black text-white px-5 py-2 rounded-full text-sm font-semibold">
              Login
            </Link>
          ) : (
            <button onClick={logout} className="bg-black text-white px-5 py-2 rounded-full text-sm font-semibold">
              Logout
            </button>
          )}
        </div>
